//! The NoETL CLI, as a library.
//!
//! Everything the `noetl` / `ntl` binaries do lives here; `src/main.rs` is a
//! four-line shim over [`cli_main`].  The split exists so embedders — the
//! PyO3 wheel published to PyPI as `noetl` being the first one — dispatch the
//! *real* clap command tree and the *real* [`PlaybookRunner`] rather than
//! reimplementing either.
//!
//! Two entry points, in order of preference for an embedder:
//!
//! - [`cli_main`] — parse an argv and run the full command tree, returning a
//!   process exit code.  This is the whole CLI; every subcommand works.
//! - [`PlaybookRunner`] — the local execution engine on its own, for callers
//!   that want a structured [`RunOutcome`] back instead of rendered output.
//!
//! Both are re-exports of the code the binary already ran; there is no second
...
/// The local execution engine and its structured outcome.
///
/// `PlaybookRunner` is what `noetl run --runtime local` drives; it dispatches
/// steps through the `noetl-tools` registry.  Re-exported at the crate root so
/// embedders get a stable path that does not depend on the module layout.
pub use playbook_runner::{PlaybookRunner, RunOutcome};
...
// The displayed command name is set at runtime from argv[0] in `cli_main`
// rather than baked in with `env!("CARGO_BIN_NAME")`: that macro only resolves
// inside a bin target, and this type now lives in the library.  Deriving the
// name from argv[0] keeps `ntl --help` self-titled `ntl`, and lets an embedder
// (the PyO3 wheel's console script) present its own name.
#[derive(Parser)]
...
fn build_variables(payload: Option<&str>, variables: &[String]) -> Result<HashMap<String, String>> {
...
fn build_distributed_payload(
    ...
    if let Some(input_file) = input {
        let content = fs::read_to_string(input_file).context(format!("Failed to read input file: {:?}", input_file))?;
        let input_json: serde_json::Value = serde_json::from_str(&content).context("Failed to parse input JSON")?;
        match input_json {
...
///
/// This is the whole `noetl` command tree — the binaries in `src/main.rs` do
/// nothing but forward `std::env::args_os()` here, and embedders (the PyO3
/// wheel) call it with a synthesised argv.  Behaviour matches the binary
/// exactly: clap renders `--help` / `--version` to stdout and usage errors to
/// stderr, and an `anyhow` error from the dispatch prints `Error: {:?}` and
/// yields exit code 1.
///
/// The caller owns the exit: this function returns a code rather than calling
/// `std::process::exit`, so an embedder can keep running afterwards.
pub fn cli_main<I, T>(argv: I) -> i32
where
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    // `noetl` and `ntl` are the same binary; the help header names whichever
    // one was invoked.  An embedder passing its own argv[0] gets the same
    // treatment.
    let program = argv
        .first()
    ...
    // `#[tokio::main]` built a multi-thread runtime; keep that shape so
    // anything relying on `spawn_blocking` or a reactor thread still works.
...
/// The body of the former `main`: everything after argv parsing.
        ...
        Some(Commands::RunLegacy {
            ...
            if effective_runtime == "distributed" {
                ...
                let catalog_path = if let Some(ref_str) = &reference {
                    ...
                    ref_str.trim_end_matches(".yaml").trim_end_matches(".yml").to_string()
                } else {
...
async fn handle_context_command(config: &mut Config, command: ContextCommand) -> Result<()> {
    match command {
        ...
        ContextCommand::List => {
            println!(
                "  {:<15} {:<30} {:<12} {:<22} {:<10}",
                "NAME", "SERVER URL", "RUNTIME", "AUTH0 DOMAIN", "TOKEN"
            );
            for (name, ctx) in &config.contexts {
                ...
                let token_state = if ctx.gateway_session_token.is_some() {
                    "cached"
                } else {
                    "-"
                };
                println!(
                    "{} {:<15} {:<30} {:<12} {:<22} {:<10}",
                    current_mark, name, ctx.server_url, ctx.runtime, auth0_domain, token_state
                );
            }
        }
                ...
                if let Some(auth0_input) = auth0 {
                    if let Some(parsed) = extract_token_from_callback_url(&auth0_input) {
                        ...
                        if use_password_grant || early_client_secret.is_some() {
                            ...
                            let domain = early_domain.ok_or_else(|| {
                                anyhow::anyhow!(
                                    "Auth0 domain not set. Use 'noetl context add --auth0-domain <domain>' first."
                                )
                            })?;
                            let client_id = early_client_id.ok_or_else(|| {
                                anyhow::anyhow!(
                                    "Auth0 client_id not set. Use 'noetl context add --auth0-client-id <id>' first."
                                )
                            })?;
                            ...
                            let auth0_resp: serde_json::Value =
                                resp.json().await.context("Failed to parse Auth0 token response")?;
                            ...
                            if !json {
                                ...
                                if let (Some(domain), Some(client_id)) = (early_domain, early_client_id) {
                                    ...
                                    println!(
                                        "After login, copy the callback URL (contains #id_token=...) and paste below."
                                    );
                                }
...
async fn get_status(
    ...
    let infer_duration_human = |result: &serde_json::Value| -> Option<String> {
        ...
        let seconds = result.get("duration_seconds").and_then(|v| v.as_f64()).or_else(|| {
            result
    ...
    let status_url = api_url(
        base_url,
        &format!("executions/{}/status", execution_id),
        use_gateway_proxy,
    );
    let response = client
        .get(&status_url)
    ...
    if response.status().is_success() {
        ...
        if json_only {
            ...
            let message = result
                .get("message")
                .and_then(|v| v.as_str())
                .unwrap_or("Cannot cancel execution");
...
async fn get_catalog_resource(client: &Client, base_url: &str, use_gateway_proxy: bool, path: &str) -> Result<()> {
...
async fn handle_iap_workspace_command(command: IapWorkspaceCommand) -> Result<()> {
    match command {
        ...
        IapWorkspaceCommand::Delete { name, remote, force } => iap_workspace_delete(&name, remote, force).await,
    }
}
...
async fn iap_workspace_list(include_remote: bool) -> Result<()> {
    ...
    let current_workspace = if state_path.exists() {
        ...
        conn.query_row("SELECT value FROM iap_config WHERE key = 'workspace'", [], |row| {
            row.get::<_, String>(0)
        })
        .ok()
    } else {
    ...
    if registry.is_empty() && current_workspace.is_none() {
        ...
        if let Some(ref current) = current_workspace {
            if !registry.contains_key(current) {
                println!("{:<20} {:<20} {:<15} *  (not in registry)", current, "-", "-");
            }
        }
    }
    ...
    if include_remote {
        ...
        let bucket = if state_path.exists() {
            let conn = Connection::open(&state_path)?;
            conn.query_row("SELECT value FROM iap_config WHERE key = 'bucket'", [], |row| {
                row.get::<_, String>(0)
            })
            .ok()
        } else {
    ...
    if entry.is_none() {
        ...
        if state_path.exists() {
            ...
            let bucket: Option<String> = conn
                .query_row("SELECT value FROM iap_config WHERE key = 'bucket'", [], |row| {
                    row.get(0)
                })
                .ok();
                ...
                if let Ok(out) = check {
                    if out.status.success() {
                        ...
                        let project: String = conn
                            .query_row("SELECT value FROM iap_config WHERE key = 'project'", [], |row| {
                                row.get(0)
                            })
                            .unwrap_or_else(|_| "unknown".to_string());
                        let region: String = conn
                            .query_row("SELECT value FROM iap_config WHERE key = 'region'", [], |row| {
                                row.get(0)
                            })
...
async fn iap_workspace_current() -> Result<()> {
    ...
    let state_path = get_state_db_path()?;
    if !state_path.exists() {
        return Err(anyhow::anyhow!("No state database found. Run 'noetl iap init' first."));
    }

    let conn = Connection::open(&state_path)?;

    let workspace: String = conn
        .query_row("SELECT value FROM iap_config WHERE key = 'workspace'", [], |row| {
            row.get(0)
        })
        .unwrap_or_else(|_| "default".to_string());
    ...
    println!(
        "  State Path:  {}",
        state_path_template.replace("{workspace}", &workspace)
    );
    println!();
    println!("State:");
...
async fn iap_workspace_create(name: &str, from: Option<&str>, switch_to: bool) -> Result<()> {
    ...
    let (project, region, bucket, state_path_template) = if let Some(source) = from {
        ...
        if let Some(entry) = registry.get(source) {
            ...
            return Err(anyhow::anyhow!("Source workspace '{}' not found in registry.", source));
        }
    } else {
...
async fn iap_workspace_delete(name: &str, delete_remote: bool, force: bool) -> Result<()> {
    ...
    if !registry.contains_key(name) {
        return Err(anyhow::anyhow!("Workspace '{}' not found in registry.", name));
    }
    ...
    if delete_remote {
        if let Some(entry) = registry.get(name) {
            if let Some(ref bucket) = entry.bucket {
                ...
                let output = std::process::Command::new("gsutil").args(["rm", &gcs_path]).output();
...
async fn iap_state_rm(resource: &str, force: bool) -> Result<()> {
    ...
    if !force {
        print!(
            "Remove '{}' from state? This does not destroy the actual resource. [y/N] ",
            resource
        );
        use std::io::{self, Write};
        io::stdout().flush()?;
...
async fn iap_state_query(sql: &str) -> Result<()> {
    ...
    let output = Command::new("duckdb").arg(&state_path).arg("-c").arg(sql).output();
...
async fn iap_sync_pull(force: bool) -> Result<()> {
    ...
    if !force {
        print!("Pull state from {}? This will overwrite local state. [y/N] ", gcs_path);
        use std::io::{self, Write};
        io::stdout().flush()?;
...
async fn iap_sync_status() -> Result<()> {
    ...
    let bucket: Result<String, _> = conn.query_row("SELECT value FROM iap_config WHERE key = 'bucket'", [], |row| {
        row.get(0)
    });
    ...
    match bucket {
        Ok(b) => {
            ...
            let output = std::process::Command::new("gsutil").args(["ls", &gcs_path]).output();
...
async fn iap_drift_report(format: &str) -> Result<()> {
    ...
    match format {
        ...
        _ => {
            println!("{:<40} {:<20} {:<15}", "RESOURCE", "DETECTED", "DRIFT TYPE");
            println!("{}", "-".repeat(80));
